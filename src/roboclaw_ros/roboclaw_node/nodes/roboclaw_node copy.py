...
class EncoderOdom:
    def __init__(self, ticks_per_meter, base_width):
        ...
        self.BASE_WIDTH = 0.42
        self.BASE_LENGTH = 0.405
    ...
    def update(self, enc_left_forw, enc_right_forw, enc_left_back, enc_right_back):
        ...
        dist_left_forw = left_forw_ticks / self.TICKS_PER_METER
        dist_right_forw = right_forw_ticks / self.TICKS_PER_METER
        dist_left_back = left_back_ticks / self.TICKS_PER_METER
        dist_right_back = right_back_ticks / self.TICKS_PER_METER
        ...
        # if left_forw_ticks == right_forw_ticks:
        #     d_theta = 0.0
        #     dist = (dist_right_forw + dist_left_forw) / 2.0
        #     dist_lat = 0.0
        #     self.cur_x += dist * cos(self.cur_theta)
        #     self.cur_y += dist * sin(self.cur_theta)
            
        # elif right_forw_ticks == -left_forw_ticks and (right_forw_ticks != 0.0 or left_forw_ticks != 0.0) :
        #     dist = 0.0
        #     d_theta = 0.0
        #     self.cur_x += dist * cos(self.cur_theta)
        #     self.cur_y += dist * sin(self.cur_theta)
        #     if left_forw_ticks > 0 and right_forw_ticks < 0 :
        #         dist_lat = (dist_left_forw - dist_right_forw) / 2.0
        #     elif right_forw_ticks > 0 and left_forw_ticks < 0:
        #         dist_lat = (dist_left_forw - dist_right_forw) / 2.0
        #     else:
        #         dist = 0
        #         dist_lat = (dist_left_forw - dist_right_forw) / 2.0
                
        d_theta = -(dist_right_forw - dist_left_forw + dist_right_back - dist_left_back) / (4 * self.BASE_WIDTH + 4 * self.BASE_LENGTH)
        dist = (dist_left_forw + dist_right_forw + dist_right_back + dist_left_back) / 4.0
        dist_lat = (dist_left_forw - dist_right_forw + dist_right_back - dist_left_back) / 4.0
        ...
        self.cur_x += 0.2 * (dist * (cos(d_theta)) - dist_lat * (sin(d_theta)))
        self.cur_y += 0.2 * (dist * (sin(d_theta)) + dist_lat * (cos(d_theta))) 
        self.cur_theta = self.normalize_angle(self.cur_theta + 0.2 * d_theta)
        ...
        if abs(d_time) < 0.000001:
            ...
            vel_x = dist / d_time
            vel_y = dist_lat / d_time
            vel_theta = d_theta / d_time
        
                    
        #print("Distancias: LF {:.4f}, RF {:.4f}, LB {:.4f}, RB {:.4f}".format(dist_left_forw, dist_right_forw, dist_left_back, dist_right_back))
        #print("Distancia lateral: {:.4f}".format(dist_lat))
        #print("Vel_y: {:.4f}".format(vel_y))
        #print("Distancia longitudinal: {:.4f}".format(dist))
        #print("Vel_x: {:.4f}".format(vel_x))
        #print("Distancia angular: {:.4f}".format(d_theta))
        #print("Vel_theta: {:.4f}".format(vel_theta))
        #print("sel cur theta: {:.4f}".format(self.cur_theta))

        return vel_x, vel_y, vel_theta, right_forw_ticks, left_forw_ticks, left_back_ticks, right_back_ticks
...
#####################################################################################################
        ...
        self.TICKS_PER_METER    = rospy.get_param("~ticks_per_meter", 26890)
        self.BASE_WIDTH         = rospy.get_param("~base_width", 0.315)
        self.CMD_FREQ           = rospy.get_param("~cmd_frequency", 30)
