...
void LaserscanMerger::pointcloud_to_laserscan(Eigen::MatrixXf points, pcl::PCLPointCloud2 *merged_cloud)
{
	...
	for (int i = 0; i < points.cols(); i++)
	{
		...
		int index = std::round((angle - output->angle_min) / output->angle_increment);
		if (index >= 0 && index < static_cast<int>(ranges_size)) {
		    if (output->ranges[index] * output->ranges[index] > range_sq)
		        output->ranges[index] = sqrt(range_sq);
		if (index < 0 || index >= static_cast<int>(ranges_size)) {
		    ROS_WARN_THROTTLE(1.0, "Índice fuera de rango: angle=%.3f, index=%d, ranges_size=%d", angle, index, ranges_size);
		    continue;
		}
}
	}
